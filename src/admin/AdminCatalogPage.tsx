...
type Project = { id: string; name: string; companyId: string; companyName: string; isActive: boolean };
...
export default function AdminCatalogPage() {
  ...
  const addCompany = async () => {
    ...
    if (!name) return;
    const res = await fetch(`${API}/admin/companies`, { method: "POST", headers, body: JSON.stringify({ name }) });
  ...
  const toggleProject = async (p: Project) => {
    const res = await fetch(`${API}/admin/projects/${p.id}`, { method: "PATCH", headers, body: JSON.stringify({ isActive: !p.isActive }) });
...
const delProject = async (id: string) => {
  ...
  const res = await fetch(`${API}/admin/projects/${id}`, { method: "DELETE", headers });
  if (!res.ok) return alert("שגיאה במחיקה");
  ...
  if (!adminKey) {
    return (
      <div className="p-6 text-center" dir="rtl">
        <div className="inline-block p-4 border rounded-xl bg-white">
    ...
    <div className="mx-auto max-w-6xl p-4" dir="rtl">
      ...
      <section className="mb-6 p-4 border rounded-2xl bg-white">
        ...
        <div className="flex gap-2">
          <input className="border rounded px-3 py-2 flex-1" placeholder="שם חברה" value={newCompany} onChange={(e)=>setNewCompany(e.target.value)} />
          <AsyncButton className="border rounded px-3 py-2" onClick={addCompany}>הוסף</AsyncButton>
        </div>
