import {
  Document, Page, Text, View, StyleSheet, Svg, Path, pdf, Font,Image
} from "@react-pdf/renderer";
import type { Stroke, SigMeta } from "../../components/SignaturePad";
import { fetchSurveyCached } from "../../../data/surveyStore";
...
export async function generateWorkLogPdfBlob(
  ...
  const survey = await fetchSurveyCached(surveyId); 
...
>
  ...
  >
    ...
    <Text style={{ fontSize: 10, textAlign: "center" }}>{survey.address || "-"} </Text>
    <Text style={{ fontSize: 10, textAlign: "center" }}>{survey.phone || "-"} </Text>
  </View>
  ...
  <View style={{ flex: 1 }}>
    ...
    <Text style={{ textAlign: "center" }}>חתימת ראש צוות</Text>
        ...
        <View style={{ position: "absolute", left: 0, right: 0, top: "45%", alignItems: "center", opacity: 0.1 }}>
          <Text style={{ fontSize: 48 }}>{survey.name}</Text>
        </View>
      </Page>
    </Document>
  );
