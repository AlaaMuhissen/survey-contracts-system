...
import { buildWorkLogWhatsAppMessage } from "../utils/worklogsummary";
...
/**
 * Send the work log PDF via WhatsApp AND persist it exactly like Save does
...
const sendWhatsApp = async (
  ...
  if (!navigator.onLine) {
    ...
    await sendPdfViaWhatsAppNoBackend(offlineBlob, filename("00000-offline"), {
      messagePrefix: `שלום,\n${buildWorkLogWhatsAppMessage(form, "זמני (אופליין)")}`,
    });
  ...
  // 6) Share via WhatsApp with the REAL number
  await sendPdfViaWhatsAppNoBackend(pdfBlob, filename(number), {
    messagePrefix: `שלום,\n${buildWorkLogWhatsAppMessage(form, number)}`,
  });
